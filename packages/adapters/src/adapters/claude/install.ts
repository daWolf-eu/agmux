...
// Copying the plugin into <configDir>/skills/agmux/ makes Claude auto-load it as
...
function readVersion(pluginDir: string): string | null {
...
export function claudeInstall(ctx: InstallContext, pluginSourceDir: string): InstallRecord {
  const dest = skillsPluginDir(resolveConfigDir(ctx));
  fs.rmSync(dest, { recursive: true, force: true }); // idempotent: refresh the copy
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  fs.cpSync(pluginSourceDir, dest, { recursive: true }); // preserves the shim's executable bit
  return {
    agentKind: "claude",
    profile: ctx.profile,
...
export function claudeStatus(ctx: InstallContext, pluginSourceDir: string): InstallStatus {
  const dest = skillsPluginDir(resolveConfigDir(ctx));
  const installedVersion = readVersion(dest);
  const installed = installedVersion !== null;
  const drift = installed && installedVersion !== readVersion(pluginSourceDir);
